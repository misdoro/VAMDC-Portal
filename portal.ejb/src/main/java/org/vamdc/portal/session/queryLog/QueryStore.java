...
public class QueryStore {
	 ...
	 * key = node id
	 */
	private Map<String, QueryStoreResponse> uuids= new HashMap<String, QueryStoreResponse>(5);
	...
	private void setUuid(String node, String result, String error){
		...
		if(error == null){
			if(!result.equals("")){
				// found
				this.uuids.put(node, new QueryStoreResponse(QueryStoreResponse.STATUS_SUCCESS, result, ""));
			} else{
				// not found
				this.uuids.put(node, new QueryStoreResponse(QueryStoreResponse.STATUS_UNKNOWN, result, ""));
			}
		}
		//failed
		else{
			this.uuids.put(node, new QueryStoreResponse(QueryStoreResponse.STATUS_ERROR, "", error));
		}		
	}
	 ...
	 */
	private String doRequest(String token, String email, String userIp) throws ClientProtocolException, IOException, HttpException{
		...
		if(response.getStatusLine().getStatusCode() == 200){
			...
			// client error
				...
				throw new HttpException("Client error : " + response.getStatusLine().getStatusCode());			
			}	
		}
	...
	private String getRequest(String token, String email, String userIp) throws UnsupportedEncodingException{
		...
		result =  result + "queryToken=" + token
					  	 + "&email=" + email 
					     + "&userIp=" + userIp
	...
	private String getIpAdress(){
		String ipAddress = ServletContexts.instance().getRequest().getHeader("X-FORWARDED-FOR");
		if (ipAddress == null) {
